...
const STORAGE_KEY = "welcomeflow-ra-clean-settings-v1";
const RECENT_KEY = "welcomeflow-ra-clean-recent-v1";
const SUBMISSIONS_KEY = "welcomeflow-ra-clean-submissions-v1";
...
function Button({ children, onClick, primary, style, type }) {
  return (
    <button type={type || "button"} onClick={onClick} style={{ padding: "11px 16px", borderRadius: 12, border: primary ? "1px solid #0f172a" : "1px solid #d7deea", background: primary ? "linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%)" : "#ffffff", color: primary ? "#ffffff" : "#0f172a", fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "Arial, sans-serif", ...style }}>
      {children}
    </button>
  );
}
...
function Field({ label, children, color }) {
  return (
    <label style={{ display: "block" }}>
      <div style={{ marginBottom: 6, fontSize: 11, fontWeight: 700, color, textTransform: "uppercase", letterSpacing: 0.5, fontFamily: "Arial, sans-serif" }}>{label}</div>
      {children}
    </label>
  );
}
...
function SelectInput({ value, onChange, options, placeholder, border }) {
  return (
    <select value={value} onChange={onChange} style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: `1px solid ${border}`, boxSizing: "border-box", background: "#ffffff", outline: "none", fontSize: 13, fontFamily: "Arial, sans-serif" }}>
...
function Card({ title, subtitle, children, action, theme }) {
  return (
    <section style={{ background: theme.cardBg, border: `1px solid ${theme.cardBorder}`, borderRadius: 20, padding: 24, boxShadow: "0 10px 26px rgba(15,23,42,0.06)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, marginBottom: 20, alignItems: "flex-start" }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 22, color: theme.text, fontFamily: "Arial, sans-serif", fontWeight: 700 }}>{title}</h2>
          {subtitle ? <p style={{ margin: "8px 0 0 0", color: theme.muted, fontSize: 13, lineHeight: 1.6, fontFamily: "Arial, sans-serif" }}>{subtitle}</p> : null}
        </div>
        {action || null}
      </div>
      {children}
...
function ToggleField({ label, checked, onChange, theme }) {
  return (
    <label style={{ display: "flex", alignItems: "center", justifyContent: "space-between", border: `1px solid ${theme.cardBorder}`, borderRadius: 12, padding: 12, background: "#ffffff" }}>
      <span style={{ fontWeight: 700, fontSize: 13, color: "#0f172a", fontFamily: "Arial, sans-serif" }}>{label}</span>
...
function NavButton({ active, children, onClick }) {
  return (
    <button type="button" onClick={onClick} style={{ width: "100%", textAlign: "left", padding: "12px 14px", borderRadius: 12, border: active ? "1px solid #1e3a8a" : "1px solid transparent", background: active ? "linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%)" : "transparent", color: active ? "#ffffff" : "#334155", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "Arial, sans-serif" }}>
      {children}
    </button>
  );
}
...
function TagListEditor({ label, values, onChange, theme }) {
  ...
  return (
    <div style={{ border: `1px solid ${theme.cardBorder}`, borderRadius: 16, padding: 16, background: "#ffffff" }}>
      <div style={{ marginBottom: 10, fontSize: 13, fontWeight: 700, color: "#0f172a" }}>{label}</div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
        {values.map((value) => <span key={value} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 9px", borderRadius: 999, background: "#eef2ff", fontSize: 12, border: "1px solid #dbe4f0" }}>{value}<button type="button" onClick={() => onChange(values.filter((item) => item !== value))} style={{ border: "none", background: "transparent", cursor: "pointer", color: "#475569" }}>×</button></span>)}
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <TextInput value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Add option" border={theme.inputBorder} />
...
function OutputBlock({ title, badge, value, theme }) {
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10, gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", color: theme.muted }}>{title}</div>
          {badge ? <span style={badgeStyle(theme.badgeBlueBg, theme.badgeBlueText)}>{badge}</span> : null}
        </div>
        <Button onClick={() => safeCopy(value)}>Copy</Button>
      </div>
      <pre style={{ margin: 0, whiteSpace: "pre-wrap", background: "#ffffff", color: "#0f172a", border: `1px solid ${theme.cardBorder}`, borderRadius: 16, padding: 18, lineHeight: 1.75, fontFamily: "Arial, sans-serif", fontSize: 13 }}>{value}</pre>
    </div>
  );
}
...
export default function App() {
  ...
  const setupProgress = useMemo(() => {
    const checks = [
      Boolean(settings.general.companyName || settings.general.recruiterName),
      settings.sites.some((site) => site.siteName),
      settings.roles.some((role) => role.positionTitle),
      !settings.compensationStructure.enabled || settings.compensationStructure.rules.some((rule) => rule.positionTitle && rule.baseAmount),
      Boolean(settings.templates.greetingLine && settings.templates.introLine),
    ];
      ...
      <div style={shellStyle}>
        <Card title={BRAND.appName} subtitle={BRAND.tagline} theme={theme} action={<div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}><Button primary={activePage === "submission"} onClick={() => setActivePage("submission")}>Submission</Button><Button primary={activePage === "submissions"} onClick={() => setActivePage("submissions")}>Submissions</Button><Button primary={activePage === "settings"} onClick={() => setActivePage("settings")}>Settings</Button></div>}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><span style={badgeStyle(theme.badgeBlueBg, theme.badgeBlueText)}>Workspace Ready</span><span style={badgeStyle(theme.badgeGreenBg, theme.badgeGreenText)}>{settings.compensationStructure.enabled ? "Compensation Active" : "Manual Compensation"}</span><span style={badgeStyle(theme.badgeSlateBg, theme.badgeSlateText)}>{activeRoles.length} Active Roles</span><span style={badgeStyle(theme.badgeGoldBg, theme.badgeGoldText)}>{activeSites.length} Active Sites</span></div>
            <div style={{ minWidth: 180 }}><SelectInput value={themeKey} onChange={(event) => setThemeKey(event.target.value)} options={[{ label: "Corporate Premium", value: "corporate" }, { label: "Dark Premium", value: "dark" }]} border={theme.inputBorder} /></div>
          </div>
        </Card>
        ...
        <div style={{ marginTop: 18, marginBottom: 24, background: theme.cardBg, border: `1px solid ${theme.cardBorder}`, borderRadius: 20, padding: 18 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: theme.muted, textTransform: "uppercase", letterSpacing: 0.5 }}>Setup Progress</div>
          <div style={{ marginTop: 8, height: 10, background: "#e2e8f0", borderRadius: 999, overflow: "hidden" }}><div style={{ width: `${setupProgress.percent}%`, height: "100%", background: `linear-gradient(135deg, ${theme.secondary} 0%, ${theme.accent} 100%)` }} /></div>
          ...
          <div style={gridTwo}>
            <div style={{ display: "grid", gap: 24 }}>
              <Card title="Candidate Intake" subtitle="Professional intake flow with work expectations and clean outputs." theme={theme} action={<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Button onClick={() => setForm(DEMO_FORM)}>Load Demo</Button><Button onClick={clearForm}>Clear Form</Button></div>}>
                <div style={fieldGrid}>
                  <Field label="Position" color={theme.muted}><SelectInput value={form.position} onChange={(event) => updateForm("position", event.target.value)} options={activeRoles.map((role) => role.positionTitle)} placeholder="Select role" border={theme.inputBorder} /></Field>
                  <Field label="Role Category" color={theme.muted}><SelectInput value={form.roleCategory || selectedRole?.roleCategory || ""} onChange={(event) => updateForm("roleCategory", event.target.value)} options={settings.options.roleTypes} placeholder="Healthcare or Other" border={theme.inputBorder} /></Field>
                  <Field label="Full Name" color={theme.muted}><TextInput value={form.fullName} onChange={(event) => updateForm("fullName", event.target.value)} border={theme.inputBorder} /></Field>
                  <Field label="Phone Number" color={theme.muted}><TextInput value={form.phoneNumber} onChange={(event) => updateForm("phoneNumber", event.target.value)} border={theme.inputBorder} /></Field>
                  <Field label="Email Address" color={theme.muted}><TextInput value={form.emailAddress} onChange={(event) => updateForm("emailAddress", event.target.value)} border={theme.inputBorder} /></Field>
                  <Field label="Location" color={theme.muted}><TextInput value={form.location} onChange={(event) => updateForm("location", event.target.value)} border={theme.inputBorder} /></Field>
                  <Field label="Site / Facility" color={theme.muted}><SelectInput value={form.siteName} onChange={(event) => updateForm("siteName", event.target.value)} options={activeSites.map((site) => site.siteName)} placeholder="Select site" border={theme.inputBorder} /></Field>
                  <Field label="Employment Type" color={theme.muted}><SelectInput value={form.employmentType} onChange={(event) => updateForm("employmentType", event.target.value)} options={settings.options.employmentTypes} border={theme.inputBorder} /></Field>
                  <Field label="Shift Preference" color={theme.muted}><SelectInput value={form.shiftPreference} onChange={(event) => updateForm("shiftPreference", event.target.value)} options={settings.options.shiftOptions} border={theme.inputBorder} /></Field>
                  <Field label="Work Type" color={theme.muted}><SelectInput value={form.workType} onChange={(event) => updateForm("workType", event.target.value)} options={settings.options.workTypes} border={theme.inputBorder} /></Field>
                  <Field label="FTE" color={theme.muted}><SelectInput value={form.fte} onChange={(event) => updateForm("fte", event.target.value)} options={FTE_OPTIONS} border={theme.inputBorder} /></Field>
                  <Field label="Years of Experience" color={theme.muted}><TextInput value={form.yearsExperience} onChange={(event) => updateForm("yearsExperience", event.target.value)} type="number" border={theme.inputBorder} /></Field>
                  <Field label="Education" color={theme.muted}><SelectInput value={form.educationLevel} onChange={(event) => updateForm("educationLevel", event.target.value)} options={settings.options.educationLevels} border={theme.inputBorder} /></Field>
                  <Field label="Compensation Requested" color={theme.muted}><TextInput value={form.compensationRequested} onChange={(event) => updateForm("compensationRequested", event.target.value)} placeholder="Optional override" border={theme.inputBorder} /></Field>
                  ...
                  <Field label="Start Date" color={theme.muted}><SelectInput value={form.startAvailability} onChange={(event) => updateForm("startAvailability", event.target.value)} options={settings.options.startAvailabilityOptions} border={theme.inputBorder} /></Field>
                  <Field label="Interview Availability" color={theme.muted}><TextInput value={form.interviewAvailability} onChange={(event) => updateForm("interviewAvailability", event.target.value)} border={theme.inputBorder} /></Field>
                  <div style={{ gridColumn: "1 / -1" }}><Field label="Start Date Notes" color={theme.muted}><TextArea value={form.startNotes} onChange={(event) => updateForm("startNotes", event.target.value)} border={theme.inputBorder} /></Field></div>
                  {isHealthcare ? <Field label="License Status" color={theme.muted}><SelectInput value={form.licenseStatus} onChange={(event) => updateForm("licenseStatus", event.target.value)} options={settings.options.licenseStatusOptions} border={theme.inputBorder} /></Field> : null}
                  {isHealthcare ? <Field label="CPR Status" color={theme.muted}><SelectInput value={form.cprStatus} onChange={(event) => updateForm("cprStatus", event.target.value)} options={settings.options.cprStatusOptions} border={theme.inputBorder} /></Field> : null}
                  {isHealthcare ? <Field label="Licensed Year" color={theme.muted}><TextInput value={form.licensedYear} onChange={(event) => updateForm("licensedYear", event.target.value)} border={theme.inputBorder} /></Field> : null}
                  <div style={{ gridColumn: "1 / -1" }}><Field label="Experience Notes" color={theme.muted}><TextArea value={form.experienceNotes} onChange={(event) => updateForm("experienceNotes", event.target.value)} border={theme.inputBorder} /></Field></div>
                </div>
                <div style={{ marginTop: 20 }}>
                  <h3 style={{ margin: 0, fontSize: 16, color: theme.text }}>Position-Based Work Expectations</h3>
                  <div style={{ ...fieldGrid, marginTop: 12 }}>
                    <Field label="Work Schedule" color={theme.muted}><TextInput value={form.workSchedule} onChange={(event) => updateForm("workSchedule", event.target.value)} border={theme.inputBorder} /></Field>
                    <Field label="Work Area" color={theme.muted}><TextInput value={form.workArea} onChange={(event) => updateForm("workArea", event.target.value)} border={theme.inputBorder} /></Field>
                    <Field label="OT Requirement" color={theme.muted}><SelectInput value={form.otRequirement} onChange={(event) => updateForm("otRequirement", event.target.value)} options={settings.options.workExpectationOptions.ot} border={theme.inputBorder} /></Field>
                    <Field label="Weekend Requirement" color={theme.muted}><SelectInput value={form.weekendRequirement} onChange={(event) => updateForm("weekendRequirement", event.target.value)} options={settings.options.workExpectationOptions.weekend} border={theme.inputBorder} /></Field>
                    <Field label="On-Call Requirement" color={theme.muted}><SelectInput value={form.onCallRequirement} onChange={(event) => updateForm("onCallRequirement", event.target.value)} options={settings.options.workExpectationOptions.onCall} border={theme.inputBorder} /></Field>
                  </div>
                </div>
                <div style={{ marginTop: 20 }}><Field label="Candidate Notes" color={theme.muted}><TextArea value={form.candidateNotes} onChange={(event) => updateForm("candidateNotes", event.target.value)} border={theme.inputBorder} /></Field></div>
                <div style={{ marginTop: 20 }}><Button primary onClick={generateOutput}>Generate Output</Button></div>
              </Card>
              <Card title="Recent Candidates" subtitle="Reload recent candidate data quickly." theme={theme}>
                <div style={{ display: "grid", gap: 10 }}>{recentCandidates.map((candidate, index) => <button key={`${candidate.fullName}-${index}`} type="button" onClick={() => setForm(candidate)} style={{ textAlign: "left", border: `1px solid ${theme.cardBorder}`, borderRadius: 14, padding: 12, background: "#ffffff", cursor: "pointer" }}><div style={{ fontWeight: 700, fontSize: 13 }}>{candidate.fullName || "Unnamed Candidate"}</div><div style={{ marginTop: 4, fontSize: 13, color: "#64748b" }}>{candidate.position || "No position"} • {candidate.siteName || "No site"}</div></button>)}</div>
              </Card>
            </div>
            <Card title="Generated Output" subtitle="Professional hiring manager email, ATS short block, and candidate confirmation email." theme={theme}>
              {!output ? <div style={{ border: `1px dashed ${theme.cardBorder}`, borderRadius: 16, padding: 28, textAlign: "center", color: theme.muted }}>Fill the form, then click Generate Output.</div> : <div style={{ display: "grid", gap: 22 }}><OutputBlock title="Hiring Manager Email" badge="Professional Format" value={output.hiringManagerEmail} theme={theme} /><OutputBlock title="ATS Summary Block" badge="Short Version" value={output.atsShort} theme={theme} /><OutputBlock title="Candidate Email" badge="Confirmation Option" value={output.candidateEmail} theme={theme} /></div>}
            </Card>
          </div>
        ) : activePage === "submissions" ? (
          <Card title="Submissions" subtitle="Submission history is kept separate from the intake page." theme={theme}>
            {!submissionHistory.length ? <div style={{ border: `1px dashed ${theme.cardBorder}`, borderRadius: 16, padding: 24, textAlign: "center", color: theme.muted }}>No submissions generated yet.</div> : <div style={{ display: "grid", gap: 12 }}>{submissionHistory.map((item) => <button key={item.id} type="button" onClick={() => { setOutput(item.output); setActivePage("submission"); }} style={{ textAlign: "left", border: `1px solid ${theme.cardBorder}`, borderRadius: 16, padding: 14, background: "#ffffff", cursor: "pointer" }}><div style={{ fontWeight: 700, fontSize: 13 }}>{item.candidate} | {item.position}</div><div style={{ marginTop: 4, color: "#64748b", fontSize: 13 }}>{item.site} • {item.date}</div></button>)}</div>}
          </Card>
        ) : (
