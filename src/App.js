...
function TagListEditor({ label, values, onChange, theme }) {
  const [draft, setDraft] = useState("const [tracker, setTracker] = useState([]);
...
export default function App() {
  ...
  function function generateOutput() {
  ...
  const newRow = {
    ...
    status: "Submitted",
    owner: "Recruiter",
    nextAction: "Follow up with facility",
    risk: "Low",
    age: 0
  };
  ...
  setHistory(prev => [
    ...
    const finalComp = form.compensationRequested || estimatedComp || "TBD";
    const generatedOutput = {<Field label="Submission Date" color={theme.muted}>
  <TextInput
    type="date"
  ...
  />
</Field> submissionDate: todayString(), finalComp, hiringManagerEmail: buildHiringManagerEmail(finalComp), atsShort: buildAtsShort(finalComp), candidateEmail: buildCandidateEmail(finalComp) };
    setOutput(generatedOutput);
  ...
  </div></div>; activePage === "submission" ? ...: activePage === "tracker" ? (
<Card title="Submission Tracker" subtitle="Control tower view of all candidates." theme={theme}>
  {!tracker.length ? (
    <div>No tracked submissions yet.</div>
  ) : (
    tracker.map(row => (
      <div key={row.id} style={{padding:12, border:"1px solid #e2e8f0", borderRadius:12, marginBottom:10}}>
        ...
        <div style={{marginTop:8}}>
          ...
          <select value={row.nextAction} style={{marginLeft:8}}>
...
)<Button primary={activePage === "tracker"} onClick={() => setActivePage("tracker")}>
  Tracker
</Button>
}