  ...
  function exportAtsCleanupCsv() {
    const rows = atsCleanupRows().map((row) => {
      ...
      const fullHistoryLog = [
        ...relatedAudit.map((event) => `${new Date(event.timestamp).toLocaleString()} | AUDIT | ${event.label} | ${event.detail}`),
        ...relatedHistory.map((event) => `${new Date(event.timestamp).toLocaleString()} | ${event.type} | ${event.subject}`),
      ].join(NL);
      return { ...row, fullHistoryLog };
    });
    downloadTextFile("welcomeflow-ats-cleanup.csv", toCsv(rows, ["candidate", "reqNumber", "position", "facility", "status", "nextAction", "submitted", "agingDays", "lastTouch", "historyCount", "fullHistoryLog"]), "text/csv");
  }
      ...
      <div style={shellStyle}>
        ...
        {activePage === "tracker" ? (
          <Card title="Submission Tracker Control Tower" subtitle="Status, owner, next action, aging, risk, and quick actions stay in one clean table." action={<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Button subtle onClick={exportTrackerCsv} disabled={!filteredTracker.length}>Export CSV</Button><Button subtle onClick={downloadSelectedSubmittals} disabled={!selectedTrackerIds.length}>Download Selected ATS Packet</Button><Button danger onClick={bulkArchiveSelected} disabled={!selectedTrackerIds.length}>Archive Selected</Button>{selectedTrackerIds.length ? <Button subtle onClick={clearTrackerSelection}>Clear Selection</Button> : null}</div>}>
            ...
            {!filteredTracker.length ? <EmptyState>No submissions yet. Generate one from the Submission page.</EmptyState> : <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 10px", minWidth: 980 }}><thead><tr>{["Select", "Candidate", "Role / Site", "Status", "Owner", "Next Action", "Aging", "Risk"].map((head) => <th key={head} style={{ textAlign: "left", color: THEME.muted, fontSize: 11, textTransform: "uppercase", letterSpacing: 0.5, padding: "0 10px" }}>{head}</th>)}</tr></thead><tbody>{filteredTracker.map((item) => <tr key={item.id} style={{ background: "#ffffff", boxShadow: "0 6px 18px rgba(16,24,40,0.04)" }}><td style={{ padding: 10, borderTopLeftRadius: 8, borderBottomLeftRadius: 8 }}><input type="checkbox" checked={selectedTrackerIds.includes(item.id)} onChange={() => toggleTrackerSelection(item.id)} /></td><td style={{ padding: 10 }}><button type="button" onClick={() => { setSelectedId(item.id); setActivePage("review"); }} style={{ border: "none", background: "transparent", padding: 0, margin: 0, color: THEME.primary2, fontWeight: 900, cursor: "pointer", textAlign: "left", fontFamily: "Inter, Arial, sans-serif", fontSize: 13 }}>{item.candidate}</button><div style={{ marginTop: 4, color: THEME.muted }}>{item.candidateEmail || "No email"}</div></td><td style={{ padding: 10 }}><strong>{item.position}</strong><div style={{ marginTop: 4, color: THEME.muted }}>{item.site}</div></td><td style={{ padding: 10 }}><SelectInput value={item.status} onChange={(event) => updateTracker(item.id, "status", event.target.value)} options={STATUS_OPTIONS} /></td><td style={{ padding: 10 }}><SelectInput value={item.owner} onChange={(event) => updateTracker(item.id, "owner", event.target.value)} options={OWNER_OPTIONS} /></td><td style={{ padding: 10 }}><SelectInput value={item.nextAction} onChange={(event) => updateTracker(item.id, "nextAction", event.target.value)} options={NEXT_ACTION_OPTIONS} /></td><td style={{ padding: 10 }}>{daysBetween(item.submissionDate)} days</td><td style={{ padding: 10 }}><Badge tone={riskFor(item)}>{riskFor(item)}</Badge></td></tr>)}</tbody></table></div>}
          </Card>
        ) : null}
