...
export default function App() {
  ...
  function addSite() { setSettings((prev) => ({ ...prev, sites: [...prev.sites, { id: makeId("site"), siteName: "", siteType: "", location: "", facilityEmail: "", status: "Active", notes: "" }] })); }
  ...
  function buildHiringManagerEmail(finalComp) {
    ...
    const topFitLine = form.experienceNotes
      ? `Top Fit: ${form.experienceNotes}`
  ...
  function openEmailDraft(kind) {
    ...
    const bodyMap = {
      ...
      candidate: output.candidateEmail,
      ats: output.atsShort,
    };
    const toAddress = kind === "hiring" ? selectedSiteEmail : kind === "candidate" ? form.emailAddress || "" : "";
    window.location.href = `mailto:${encodeURIComponent(toAddress)}?subject=${encodeURIComponent(subjectMap[kind])}&body=${encodeURIComponent(bodyMap[kind])}`;
  }
  ...
  function downloadTemplate(kind) {
    const templates = {
      locations: `siteName,siteType,location,facilityEmail,status,notes${NL}Demo Facility,24-hour,Douglasville GA,hiringmanager@email.com,Active,`,
  ...
  return <div style={pageStyle}><div style={shellStyle}>
    <Card title={BRAND.appName} subtitle={BRAND.tagline} theme={theme} action={<div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}><Button primary={activePage === "submission"} onClick={() => setActivePage("submission")}>Submission</Button><Button primary={activePage === "submissions"} onClick={() => setActivePage("submissions")}>Submission Tracker</Button><Button primary={activePage === "settings"} onClick={() => setActivePage("settings")}>Settings</Button></div>}>
