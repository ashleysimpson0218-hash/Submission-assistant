...
const ARCHIVE_KEY = "welcomeflow-final-v2-archive";
...
export default function App() {
  ...
  function buildAtsUpdate(row) {
    return [
      `ATS Update: ${row.candidate} | ${row.position}`,
      `Facility: ${row.facility}`,
      `Current Status: ${row.status}`,
      `Days in Current Status: ${daysBetween(row.submittedDate)}`,
      `Next Action: ${row.nextAction || "N/A"}`,
      `Client Feedback: ${row.clientFeedback || "Pending"}`,
  ...
  function generateWeeklyReport() {
    const activeRows = tracker.filter((row) => !["Placed", "Rejected", "Closed"].includes(row.status));
    const counts = settings.options.outcomes.reduce((acc, outcome) => ({ ...acc, [outcome]: tracker.filter((row) => row.outcome === outcome).length }), {});
    const body = [
      `Weekly Submission Report`,
      "",
      `Total Active Submissions: ${activeRows.length}`,
      `High Risk Aging: ${tracker.filter((row) => trackerRisk(row) === "High").length}`,
      "",
      "Outcome Summary",
      ...Object.entries(counts).map(([key, value]) => `• ${key}: ${value}`),
      "",
      "Active Submission Details",
      ...activeRows.map((row) => `• ${row.submittedDate} | ${row.candidate} | ${row.position} | ${row.facility} | ${row.status} | ${daysBetween(row.submittedDate)} day(s) in process | ${row.nextAction || "N/A"}`),
      "",
      "Please review and advise on any submissions pending feedback or next steps.",
    ].join(NL);
    setWeeklyReport({ subject: `Weekly Submission Report | ${todayString()}`, body });
  }

  function requestArchive(row) {
    setArchiveDraft({ ...row, archiveReason: row.reason || "", futureStatus: "Candidate still viable for future review", archiveNotes: "" });
  }

  function confirmArchive() {
    if (!archiveDraft?.outcome || archiveDraft.outcome === "Pending") { alert("Select a final outcome before archiving."); return; }
    if (!archiveDraft.archiveReason) { alert("Add an archive reason before archiving."); return; }
    const archivedRecord = { ...archiveDraft, archivedDate: new Date().toISOString().slice(0, 10), status: "Archived" };
    setArchived((prev) => [archivedRecord, ...prev]);
    setTracker((prev) => prev.filter((row) => row.id !== archiveDraft.id));
  ...
  const m = metrics();
  const followUpCount = tracker.filter((row) => ["Awaiting Client Review", "Follow-Up Due"].includes(row.status) && daysBetween(row.submittedDate) >= Number(settings.workflow.defaultFollowUpDays || 2)).length;
  const interviewsTodayCount = tracker.filter((row) => row.status === "Interview Scheduled" && row.dueDate === new Date().toISOString().slice(0, 10)).length;
  const managerFeedbackCount = tracker.filter((row) => ["Interview Requested", "Interview Scheduled", "Awaiting Client Review", "Offer Pending"].includes(row.status) && !row.clientFeedback).length;
  ...
  return <div style={pageStyle}><div style={shellStyle}>
    <Card title={BRAND.appName} subtitle={BRAND.tagline} theme={theme} action={<div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><Button primary={activePage === "submission"} onClick={() => setActivePage("submission")}>Submission</Button><Button primary={activePage === "tracker"} onClick={() => setActivePage("tracker")}>Submission Tracker</Button><Button primary={activePage === "archive"} onClick={() => setActivePage("archive")}>Archived Candidates</Button><Button primary={activePage === "metrics"} onClick={() => setActivePage("metrics")}>Metrics</Button><Button primary={activePage === "settings"} onClick={() => setActivePage("settings")}>Settings</Button></div>}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}><div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><DashboardChip label="Workspace" value="Ready" bg={theme.blueBg} color={theme.blueText} onClick={() => { setActivePage("settings"); setActiveSettingsTab("general"); }} /><DashboardChip label="Rate Rules" value={settings.compensationRules.length} bg={theme.greenBg} color={theme.greenText} onClick={() => { setActivePage("settings"); setActiveSettingsTab("compensation"); }} /><DashboardChip label="Tracked" value={tracker.length} bg={theme.goldBg} color={theme.goldText} onClick={() => setActivePage("tracker")} /><DashboardChip label="High Risk" value={m.highRisk} bg={theme.redBg} color={theme.redText} onClick={() => setActivePage("tracker")} /><DashboardChip label="Follow-Ups" value={followUpCount} bg={theme.slateBg} color={theme.slateText} onClick={() => setActivePage("tracker")} /><DashboardChip label="Interviews Today" value={interviewsTodayCount} bg={theme.blueBg} color={theme.blueText} onClick={() => setActivePage("tracker")} /><DashboardChip label="Manager Feedback" value={managerFeedbackCount} bg={theme.goldBg} color={theme.goldText} onClick={() => setActivePage("tracker")} /></div><div style={{ minWidth: 190 }}><SelectInput value={themeKey} onChange={(e) => setThemeKey(e.target.value)} options={[{ label: "Corporate Premium", value: "corporate" }, { label: "Dark Premium", value: "dark" }]} border={theme.inputBorder} /></div></div>
    </Card>
    ...
    {activePage === "submission" ? <div style={{ display: "grid", gap: 24, gridTemplateColumns: "1.05fr 0.95fr", alignItems: "start" }}>
      <div style={{ display: "grid", gap: 24 }}>
        <Card title="Notes-to-Profile Intake" subtitle="Paste call notes or a transcript. The app extracts a draft profile so the recruiter edits instead of starting blank." theme={theme} action={<div style={{ display: "flex", gap: 8 }}><Button onClick={previewNotes}>Preview Extract</Button>{notesPreview ? <Button primary onClick={applyNotesPreview}>Apply to Form</Button> : null}</div>}><Field label="Recruiter Notes / Transcript" color={theme.muted}><TextArea value={form.recruiterNotes} onChange={(e) => updateForm("recruiterNotes", e.target.value)} placeholder="Example: Ashley Martin RN 10 yrs corrections med-surg, active license and CPR, needs two weeks notice, prefers day shift..." border={theme.inputBorder} minHeight={130} /></Field>{notesPreview ? <div style={{ marginTop: 16, background: "#ffffff", color: "#0f172a", border: `1px solid ${theme.cardBorder}`, borderRadius: 16, padding: 16 }}><strong>Extracted Draft Preview</strong><div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10, marginTop: 12, fontSize: 13 }}><div><b>Name:</b> {notesPreview.fullName || "N/A"}</div><div><b>Role:</b> {notesPreview.position || "N/A"}</div><div><b>Experience:</b> {notesPreview.yearsExperience || "N/A"} yrs</div><div><b>Shift:</b> {notesPreview.shiftPreference || "N/A"}</div><div><b>Start:</b> {notesPreview.startAvailability || "N/A"}</div><div><b>Credentials:</b> {[notesPreview.licenseStatus, notesPreview.cprStatus].filter(Boolean).join(" | ") || "N/A"}</div><div style={{ gridColumn: "1 / -1" }}><b>Experience Notes:</b> {notesPreview.experienceNotes || "N/A"}</div></div></div> : null}</Card>
