...
export { validateCandidateReadyFacilitySubmissionPackage } from "./candidateReadyPackageValidation";
...
function uniqueEmails(values = []) {
  const normalizedValues = Array.isArray(values)
    ...
    : values == null
      ...
      : [values];
  const seen = new Set();
  return normalizedValues.flatMap((value) => String(value ?? "").split(/[;,]/)).map(text).filter((email) => {
